...
provaweather: function provaweather(){
	return new Promise(async (resolve, reject) => {
		if( openAPIweather == openAPIweather && weatherAPIweather == weatherAPIweather && bitAPIweather == bitAPIweather && underAPIweather == underAPIweather && accuAPIweather == accuAPIweather ){
			let thenVal = await createService.printLcd(openAPIweather); 
			resolve(thenVal)
		} 
		else{

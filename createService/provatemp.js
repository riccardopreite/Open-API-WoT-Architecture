...
provatemp: function provatemp(){
	return new Promise(async (resolve, reject) => {
		if( openAPItemperature == openAPItemperature && weatherAPItemperature == weatherAPItemperature && bitAPItemperature == bitAPItemperature && underAPItemperature == underAPItemperature && accuAPItemperature == accuAPItemperature ){
			let thenVal = await createService.printLcd(openAPItemperature); 
			resolve(thenVal)
		} 
		else{

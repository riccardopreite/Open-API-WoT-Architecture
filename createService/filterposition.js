...
filterposition: function filterposition(){
	return new Promise(async (resolve, reject) => {
		if(  openAPItemperature == openAPItemperature   ){
			let thenVal = await createService.filterPosition(44,11,1,'filterposition','tomtomAPI'); 
			resolve(thenVal)
		} 
		else{
			let elseVal = await createService.filterPosition(42,13,1,'filterposition','tomtomAPI'); 
			resolve(elseVal)
		}
	 });
} 
